...
MoneyPouchは7つのテーマを提供しています。各テーマは専用の背景画像（WebP最適化）とカラースキームを持っています。テーマは設定画面から変更できます。

### 利用可能なテーマ（順番）

1. **デフォルト** (`default`)
   - 背景画像: `default.webp`（ゴールド/オレンジの抽象グラデーション）
   - アイコン: `computer`
   - プライマリカラー: オレンジゴールド（`#fb923c`）

2. **ミッドナイト** (`midnight`)
   - 背景画像: `midnight.webp`（ダークブルーの抽象グラデーション）
   - アイコン: `nights_stay`
   - プライマリカラー: スカイブルー（`#38bdf8`）

3. **ネオン** (`neon`)
   - 背景画像: `neon.webp`（青紫→ピンクのネオングラデーション）
   - アイコン: `gradient`
   - プライマリカラー: シアン（`#06b6d4`）
...
4. **ハッカー** (`terminal`)
   - 背景画像: なし（純黒背景）
   - アイコン: `terminal`
   - プライマリカラー: ネオングリーン（`#00ff41`）
   - テキストもハッカー風の緑色

5. **富士山** (`fujisan`)
   - 背景画像: `fujisan.webp`（夕焼けと富士山の写真）
   - アイコン: `landscape`
   - プライマリカラー: コーラルレッド（`#ff6b6b`）

6. **夜景（横浜）** (`yokohama`)
   - 背景画像: `yokohama.webp`（みなとみらいの夜景）
   - アイコン: `night_sight_auto`
   - プライマリカラー: ローズピンク（`#ec4899`）

7. **夜景（東京）** (`tokyo`)
   - 背景画像: `tokyo.webp`（東京駅の夜景）
   - アイコン: `location_city`
   - プライマリカラー: アンバー（`#fbbf24`）

### テーマで変更される要素

以下の要素はテーマによって変わります：

- **背景画像** (`--bg-image`) - WebP形式でモバイル最適化済み（480px幅、品質85%）
